...
#   정본 = data-server/processing/transforms_B.py (SCENARIO env 디스패처가 processing/ 정본만 로드).
#   이 파일은 디스패처 도입 전 옛 per-시나리오 사본 — 같은 폴더에 transforms_B 가 없어 import 시 깨진다.
...
# ═════════════════════════════════════════════════════════════════════════════
GRAVITY     = 9.81
PITCH_LIMIT = 0.3   # deg  v10: 멀미 방지 — 피치 변화 ±0.3deg 로 강하게 제한
...
SCALE_SURGE_NORMAL = 1.0
SCALE_SWAY_NORMAL  = 1.0
SCALE_HEAVE_NORMAL = 0.5
SCALE_ROLL_NORMAL  = 0.12   # v13: 0.4→0.12  코너 banking 기울어짐 대폭 억제
SCALE_PITCH_NORMAL = 0.04   # v11: 0.15→0.04 오르막/내리막 지형 pitch 거의 무시 (자연스럽게)
SCALE_YAW_NORMAL   = 0.3
...
SCALE_PITCH_EVENT = 0.04   # v11: 0.15→0.04 지형 pitch 무시 (오르막/내리막 스파이크 제거)
...
#                    주면 뭉개짐 → 보조 수준으로만 유지.
SWAY_SHAKE_GAIN  = 3.5     # ★ 좌우흔들림 세기 핵심 노브 (v11: 2.0→3.5, 더 세게: ~5.0)
ROLL_SHAKE_GAIN  = 0.25    # v13: 0.5→0.25  진동 중 roll 기울임 최소화

# 합산 안전 상한 — heave/sway 신호가 폭주하지 않도록 클램프.
HEAVE_SIGNAL_CAP = 2.0 * 2.5   # heave 합산 안전 상한 (모든 케이스 커버)
SWAY_SIGNAL_CAP  = 9.0     # v11: 큰 좌우흔들림 허용. 너무 약하면 ↑
...
ROAD_BUZZ_HEAVE_GAIN = 1.3    # v25: 2.8→1.3  수직 진폭↓ (덜컹거리는 거침 제거 — 좌우 떨림이 主)
ROAD_BUZZ_SWAY_SCALE = 0.45   # v26: 0.85→0.45  진폭↓ → 큰 "휙" 스윙 아닌 잔 "ㄷㄷㄷ" 떨림
...
SLIP_SWAY_A = 2.8    # 슬립 횡가속 피크 [m/s²] — 한쪽으로 한 번 미끄러지는 세기
SLIP_YAW_A  = 0.6    # 슬립 중 차체 회전 [deg] — 미끄러지며 살짝 돎 (작게)
...
GRADE_SURGE_GAIN = 0.18    # m/s² per deg 내리막 (경사 1도당 가속감)
